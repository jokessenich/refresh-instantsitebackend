...
export async function runFullPipeline(
  ...
  try {
    ...
    logger.info("Starting full pipeline", {
      ...
      stockImageCount: images.stockImageUrls?.length ?? 0,
    });
    ...
    logger.info("Images processed", {
      requestId,
      fileNames,
      stockImageUrls: images.stockImageUrls?.length ?? 0,
    ...
    const html = await generateSiteHtml(input, fileNames, images.stockImageUrls);
