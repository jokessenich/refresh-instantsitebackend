...
export function buildPrompt(
  ...
  const hasUploaded = imageFileNames && imageFileNames.length > 0;
  const hasStock = stockImageUrls && stockImageUrls.length > 0;
...
IMPORTANT: These image files will be deployed alongside the HTML. Reference them with a leading slash (e.g. src="/logo.png", src="/image-1.jpg"). Do NOT use base64, external URLs, or placeholder images. Use ONLY the provided image files listed above.`;
    ...
    const imageList = stockImageUrls
      .map((url, i) => `Image ${i + 1}: ${url}`)
      .join("\n");

    imageSection = `Stock Images (use these EXACT URLs as img src attributes — do NOT modify them):
${imageList}

IMPORTANT: Use these images throughout the site — hero, about, services, gallery, and any other sections that benefit from photography. Use object-fit: cover, appropriate sizing, and good visual placement. Spread them across different sections for variety. Do NOT use any other image URLs or placeholder images. Only use the URLs listed above.`;
  } else {
    imageSection = `Images Provided:
None available. Use CSS gradients, patterns, or solid colors for visual interest. Do NOT use placeholder image URLs or broken image references.`;
  }
...
export async function generateSiteHtml(
  ...
  const prompt = buildPrompt(input, imageFileNames, stockImageUrls);
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      logger.info("Claude HTML generation successful", {
        ...
        stockImageCount: stockImageUrls?.length ?? 0,
      });
